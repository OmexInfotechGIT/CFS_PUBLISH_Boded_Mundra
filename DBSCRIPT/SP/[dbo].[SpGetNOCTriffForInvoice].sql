...
ALTER PROC [dbo].[SpGetNOCTriffForInvoice]                  
@BillDate DATE ,                    
@TeriffHeadID int,                    
@SubCommodityID int,                    
@BillCommodityID int,                    
...
BEGIN                    
 ...
 DECLARE @InterMstrCustomerID INT    
 DECLARE @InterMstrAddressesID INT    
    ...
    DECLARE @InterNOCID INT           
 DECLARE @trnDocumentLotDetailsID int          
 ...
 SET @InterMstrCustomerID ='+CAST(@MstrCustomerID AS VARCHAR(255))+'    
 SET @InterMstrAddressesID='+CAST(@MstrAddressesID AS varchar(255))+'    
    ...
    SET @InterNOCID    = '+ CAST(@NOCID AS VARCHAR(255))+'          
  SET @trnDocumentLotDetailsID    = '+ CAST(@trnDocumentLotDetailsID AS VARCHAR(255))+'          
 ...
 ISNULL((SELECT ISNULL(SUM(ISNULL(Rate,0)),0) from MstrTariffHead T           
     ...
     WHERE T.flagdeleted = 0 and T.MstrTariffHeadID = @InterTeriffHeadID     
     and SST.StateID = (SELECT TOP 1 StateId FROM MstrAddresses A     
         WHERE A.MstrCustomerID = @InterMstrCustomerID  AND A.MstrAddressesID=@InterMstrAddressesID  AND A.Flagdeleted = 0 )),0) as GSTPERForInvoice ,    
    * FROM NOCTarrifData G                    
    WHERE G.MstrTariffHeadID = @InterTeriffHeadID                    
    AND G.trnDocumentID = @InterNOCID            
 --AND (case when ''Bond-GR''=@InterBillType or ''Bond''=@InterBillType then (@trnDocumentLotDetailsID)  else -1 end )=@trnDocumentLotDetailsID          
 and (case when ''Bond-GR''=@InterBillType or ''Bond''=@InterBillType then  G.trnDocumentLotDetailsID else 1 end = case when ''Bond-GR''=@InterBillType or ''Bond''=@InterBillType then @trnDocumentLotDetailsID else 1 end )        
 ...
 AND (CASE WHEN G.StoragePattern = ''0'' THEN ''0'' ELSE  G.StoragePattern END = CASE WHEN ISNULL(@InterPattern,NULL) is null THEN ''0'' ELSE @InterPattern END           OR CASE WHEN G.StoragePattern = ''0'' THEN ''0'' ELSE  G.StoragePattern END = ''0'' )
    ...
    AND (CASE WHEN G.MstrBillableCommodityID = 0 THEN -1 ELSE  G.MstrBillableCommodityID END = CASE WHEN @InterBillCommodityID is null THEN -1 ELSE @InterBillCommodityID END  OR CASE WHEN G.MstrBillableCommodityID = 0 THEN -1 ELSE  G.MstrBillableCommodity
ID  END = -1 )                    
