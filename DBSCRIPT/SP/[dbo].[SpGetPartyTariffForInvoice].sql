...
GO
/****** Object:  StoredProcedure [dbo].[SpGetPartyTariffForInvoice]    Script Date: 5/10/2024 6:36:27 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROC [dbo].[SpGetPartyTariffForInvoice]            
@BillDate DATE ,              
@TeriffHeadID int,              
@SubCommodityID int,              
@BillCommodityID int,              
...
BEGIN              
 ...
 SELECT DATEADD(DAY,fromDays-1, CASE WHEN @InterDestuffDate !='''' AND (@InterBillType=''Bond-GR'')           
  ...
  WHERE T.flagdeleted = 0 AND T.MstrTariffHeadID = @InterTeriffHeadID        
   ...
   ,*        
 FROM PartyTarrifData G                     
    WHERE G.MstrTariffHeadID = @InterTeriffHeadID              
    AND G.MstrCustomerID = @InterMstrCustomerID              
