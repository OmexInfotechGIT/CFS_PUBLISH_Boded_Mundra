...
ALTER TRIGGER [dbo].[TGR_trnWorkOrderOut]   
 ON  [dbo].[trnWorkOrderOut]   
   ...
   Begin
	...
	IF NOT EXISTS (SELECT * FROM Inserted p  
		...
		p.[trnWorkOrderOutID] = i.[trnWorkOrderOutID]	
		...
		and p.BackToTown = i.BackToTown
		and p.[YearID] = i.[YearID]
	)   
	BEGIN       
		SET NOCOUNT ON;       
		INSERT INTO [CFS_BONDED_WAREHOUSE_History].[dbo].[trnWorkOrderOut_History]
							([trnWorkOrderOutID],[LocationFrom],[LocationFromID],
							...
							[UpdatedDate],[caption], IsFinished,BackToTown,[YearID])

					SELECT [trnWorkOrderOutID],[LocationFrom],[LocationFromID],
							[WeighmentRequired],[NonWeighmentReason],[WeighmentPaymentMode],[ManualSSRNo],[TransportatiOnType],[VehicleType],
							[TruckNo],[TruckID],[Transporter],[TransporterID],[Remarks],[Flagdeleted],[Createdby],[CreatedDate],[UpdatedBy],
							[UpdatedDate],@Caption, IsFinished,BackToTown,[YearID] FROM Inserted 
	END    
	...
	BEGIN
			...
			DECLARE cur CURSOR STATIC FOR 
			 select trnWorkOrderOutLotDetailsID from trnWorkOrderOutLotDetails WL where WL.trnWorkOrderOutID = (select trnWorkOrderOutID from Inserted)
			OPEN cur
			IF @@CURSOR_ROWS > 0
			BEGIN  FETCH NEXT FROM cur INTO @Id WHILE @@Fetch_status = 0  BEGIN
			
				IF EXISTS(select * from SSRInvoice where Infoid = @Id and InfoTableName = 'trnWorkOrderOutLotDetails' and flagdeleted = 0)
				BEGIN
						Update SSRInvoice SET Flagdeleted = 1 where Infoid = @Id and InfoTableName = 'trnWorkOrderOutLotDetails'
				END
